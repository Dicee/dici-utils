...
public class PairRichIterator<K,V> extends RichIterator<Pair<K,V>> {
	...
	public static <X,K,V> PairRichIterator<K,V> pairIterator(Iterator<X> it, ThrowingFunction<X,K> keyFunction, ThrowingFunction<X,V> valueFunction) {
		notNull(keyFunction);
		notNull(valueFunction);
		return new PairRichIterator<>(RichIterators.wrap(it).map(x -> new Pair<>(keyFunction.apply(x), valueFunction.apply(x))));
	}
