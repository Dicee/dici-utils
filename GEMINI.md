...
- **naming conventions**: I favor the use of concise test case names, hierarchical when needed. This applies to method names, I am not necessarily asking you to created nested JUnit5 classes. You can for particularly complex classes to test if there are clearly separate use cases to split by, but in general I find it overkill. I left you some examples below. Typically, I do not include the expected output of the test in the name, this should be clear from the test. The test's name is there to structure our test cases and help us see the cases and subcases that are being tested.
...
- **exceptions**: most of the time you'll want to use `com.dici.testing.assertj.BetterAssertions.assertThatThrownBy` (with a static import) instead of the AssertJ version, because this one offers the `isLike` method which allows simply passing an exception for it to be recursively compared to the thrown exception